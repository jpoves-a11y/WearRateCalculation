...
-   **Unworn Sphere Fitting**: A highly accurate sphere is fitted *only* to the unworn regions of the prosthesis using a Gauss-Newton algorithm with Levenberg-Marquardt fallback, providing a sub-micrometer precision reference for the ideal original geometry.
...
-   **Linear Wear Metrics**: Provides mean, max, and min penetration depths for worn vertices relative to the unworn reference sphere.
