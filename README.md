...
#### 2.2b Método Dual-Sphere (Convergencia de Normales) - v3.1

**Concepto**: Clasificar vértices basándose en hacia dónde convergen sus vectores normales. En una esfera perfecta, todas las normales convergen al centro. En una superficie desgastada, las normales de zonas desgastadas convergen a un centro DESPLAZADO.

**Algoritmo Ray-Based Clustering (v3.1)**:

1. **Creación de Rayos**: Para cada vértice, crear un rayo definido por:
   - **Origen**: posición del vértice
   - **Dirección**: normal hacia adentro (apuntando al interior de la copa)
   ```javascript
   ray = { origin: vertex.position, direction: inwardNormal }
   ```

2. **Inicialización de Clusters**:
   - **Cluster 0**: centro en el centro de la esfera de referencia (estimación inicial)
   - **Cluster 1**: centro desplazado hacia la dirección del desgaste más profundo
   ```javascript
   clusterCenters[0] = sphereCenter
   clusterCenters[1] = sphereCenter + wornDirection * (maxPenetration * 2)
   ```

3. **Clustering Iterativo (k-means adaptado)**:
   - **Asignación**: cada rayo se asigna al cluster cuyo centro está más cercano (por distancia perpendicular)
   - **Actualización**: recalcular centros de clusters usando intersección de rayos por mínimos cuadrados
   ```javascript
   // Fórmula de intersección óptima de rayos:
   P = (Σ(I - D⊗D))^(-1) × Σ((I - D⊗D) × O)
   // Donde D = dirección del rayo, O = origen del rayo, I = matriz identidad
   ```
   - Converge típicamente en 5-15 iteraciones

4. **Clasificación Worn/Unworn (v3.1 - Lógica Corregida)**:
   ```javascript
   // El cluster cuyo centro está MÁS LEJOS de la esfera de referencia = UNWORN
   // El cluster cuyo centro está MÁS CERCA de la esfera de referencia = WORN
   const unwornCluster = dist0ToSphere >= dist1ToSphere ? 0 : 1;
   ```

**Razonamiento Físico de la Clasificación**:

La lógica puede parecer contra-intuitiva, pero tiene fundamento geométrico:

1. **Centro de Referencia Mixto**: El centro de la esfera de referencia se calcula usando TODOS los vértices no-rim (una mezcla de desgastados y no desgastados), por lo que es un "centroide mixto"

2. **Normales Unworn**: Convergen al centro VERDADERO de la esfera original de la prótesis, que está típicamente MÁS LEJOS de este centroide mixto

3. **Normales Worn**: La zona desgastada es una superficie "aplanada" o erosionada. Sus normales convergen a un centro DESPLAZADO hacia la zona de desgaste, que está MÁS CERCA del centroide mixto de la copa

4. **Conclusión**: `FARTHER from reference = UNWORN`, `CLOSER to reference = WORN`

**Visualización**:
- **Marcador Verde**: Centro de convergencia de normales unworn
- **Marcador Rojo**: Centro de convergencia de normales worn
- La distancia entre centros representa el desplazamiento lineal por desgaste

**Ventajas del Método Dual-Sphere**:
- Independiente de la posición geométrica absoluta
- Basado en la dirección de las normales (propiedad local de la superficie)
- Robusto a variaciones de orientación de la malla
- Proporciona directamente el desplazamiento del centro (estimación de desgaste lineal)

**Cuándo Usar Dual-Sphere vs Hemisphere**:
- **Dual-Sphere**: Cuando el desgaste es claramente localizado y las normales de la zona desgastada son consistentes
- **Hemisphere**: Para análisis general cuando no se requiere información sobre centros desplazados
