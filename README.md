...
**Method**: Volumetric integration of positive penetrations across entire inner surface (v3.2).

**Algorithm**:
1. **Surface Processing**: Process ALL triangles in the inner surface (independent of worn/unworn classification)

2. **Triangle Clipping**: Clip each triangle by the transition plane to retain only the portion within the bounded region
   - Plane normal points inward (toward acetabular center)
   - Distance ≤ 0 indicates points on the worn side (toward center)

3. **Penetration Calculation**: For each clipped triangle vertex:
   - Calculate penetration depth = sphere_radius - distance_to_center
   - Positive penetration = point inside sphere (material loss)
   - Negative penetration = point outside sphere (preserved material)

4. **Volume Integration**: For each clipped triangle with positive average penetration:
   - Calculate average penetration over the triangle
   - Volume contribution = average_penetration × triangle_area
   - Sum all positive contributions (ignore negative)

**Formula**: V_wear = Σ (penetration_avg × area) for ALL triangles with penetration_avg > 0

**Physical Interpretation**: 
- Measures total material loss across entire surface within plane boundary
- Independent of worn/unworn classification (more objective)
- Captures all regions where real surface lies inside fitted reference sphere
- Integration space bounded by transition plane (toward inner surface)

**Advantages**:
- Not dependent on classification accuracy
- Measures true geometric deviation from ideal sphere
- More robust and objective measurement
...
- **v3.2** (2025-11-25): Improved volumetric wear calculation to process ALL triangles with positive penetration (classification-independent, more robust)
